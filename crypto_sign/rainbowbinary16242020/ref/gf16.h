...
const byte gf16_inv_tab[] = 
{ 0, 1, 9, 14, 13, 11, 7, 6, 15, 2, 12, 5, 10, 4, 3, 8 };
	
const byte gf16_mul_tab[] = {
...
template <>
void gfv_unit<16>::dump(FILE * fp) const 
{ 
	for(unsigned i=0;i<N;i++) fprintf(fp,"%02X ",(int)((*this)[i].v)); fprintf(fp,"\n");
}

template <>
inline const gfv_unit<16> gfv_unit<16>::rand() { return _mm_and_si128( _mm_set_epi32(::rand(),::rand(),::rand(),::rand()),_mask_low );  }


inline const gfv_unit<16> _mul_safe( const gfv_unit<16> &a , const gf<16> & b )
{
	return _mm_shuffle_epi8(multable(b.v), _mm_and_si128(a.v,_mask_low));
}


template <>
inline const gfv_unit<16> _mul( const gfv_unit<16> &a , const gf<16> & b )
{
	return _mm_shuffle_epi8(multable(b.v), a.v);
}

template <>
inline const gf<16> _mul( const gf<16> &a , const gf<16> &b)
{
...
template <>
inline const gf <16> _inv( const gf <16> &a )
{
...
//template <>
//inline const gfv_unit<16> _rand_gfv_unit() { return _mm_and_si128( _mm_set_epi32(::rand(),::rand(),::rand(),::rand()),_mask_low ); }


static const __m128i m_bit0 = _mm_set1_epi8(0x11);
...
inline __m128i _gf16v_mul( __m128i a , __m128i b )
{
	__m128i & a0= a;
	__m128i & b0= b;
	__m128i a1=SR(a,1);
	...
	return OR(OR(r0,SL(r1,1)),OR(SL(r2,2),SL(r3,3)));
}
#undef AND
#undef OR
#undef XOR
...
template <>
inline const gfv_unit<16> _mul( const gfv_unit<16> & a, const gfv_unit<16> &b )
{
	return _gf16v_mul( a.v , b.v );
}
...
inline const gfv_unit<16> _mul( const gfv_unit<16> & a, const gfv_unit<16> &b )
{
	...
	__m128i r;
	byte *_r = (byte*)&r;
	for(unsigned i=16;i--;) _r[i]=gf16_mul_tab[((_a[i]&0xF)<<4)+(_b[i]&0xF)];
	return r;
}
*/
