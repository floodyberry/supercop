...
const u32 aes_big_constants[1034] = {
0x01000000,
0x02000000,
0x04000000,
0x08000000,
...
#define table0 ((char *) &aes_big_constants[10])
#define table1 ((char *) &aes_big_constants[12])
...
{
  c->data[14] = U8TO32_BIG(iv);
  c->data[15] = U8TO32_BIG(iv + 4);
  c->data[16] = U8TO32_BIG(iv + 8);
  c->data[17] = U8TO32_BIG(iv + 12);
}
...
void ECRYPT_process_bytes_C(int action,ECRYPT_ctx *c,const u8 *input,u8 *output,u32 len)
{
  ...
  while (len > 0) {
    ...
    if (len < 16) {
      ...
      U32TO8_BIG(tmp + 4,y1);
      U32TO8_BIG(tmp + 8,y2);
      U32TO8_BIG(tmp + 12,y3);
      for (i = 0;i < len;++i) output[i] = input[i] ^ tmp[i];
      break;
    }

    y0 ^= U8TO32_BIG(input + 0);
    y1 ^= U8TO32_BIG(input + 4);
    y2 ^= U8TO32_BIG(input + 8);
    y3 ^= U8TO32_BIG(input + 12);


    U32TO8_BIG(output + 0,y0);
    U32TO8_BIG(output + 4,y1);
    U32TO8_BIG(output + 8,y2);
    U32TO8_BIG(output + 12,y3);
