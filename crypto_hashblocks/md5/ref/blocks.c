...
#define block(i) (load_littleendian(in + 4 * (i)))
...
int crypto_hashblocks(unsigned char *statebytes,const unsigned char *in,unsigned long long inlen)
{
  ...
  a = load_littleendian(statebytes +  0); state[0] = a;
  b = load_littleendian(statebytes +  4); state[1] = b;
  c = load_littleendian(statebytes +  8); state[2] = c;
  d = load_littleendian(statebytes + 12); state[3] = d;
  ...
  store_littleendian(statebytes +  0,state[0]);
  store_littleendian(statebytes +  4,state[1]);
