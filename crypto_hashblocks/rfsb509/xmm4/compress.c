...
#define matrix crypto_hashblocks_rfsb509_xmm4_matrix
...
int crypto_hashblocks(unsigned char *r,const unsigned char *b,unsigned long long blen)
{
  ...
  while (blen >= 48) {
    ...
    for (i = 0;i < 51;++i) {
      u = c[i];
      c[i + 64] ^= _mm_slli_si128(u,6);
      c[i + 63] ^= _mm_srli_si128(u,10);
    }

    for (i = 51;i < 83;++i) {
      u = c[i];
      c[i + 32] ^= _mm_slli_si128(u,3);
      c[i + 31] ^= _mm_srli_si128(u,13);
    }
