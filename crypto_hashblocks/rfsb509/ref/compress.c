...
static void column_load(column *c, unsigned char pos)
{
  ...
  c->x[0] = load_littleendian(matrix + p +  0);
  c->x[1] = load_littleendian(matrix + p +  8);
  c->x[2] = load_littleendian(matrix + p + 16);
  c->x[3] = load_littleendian(matrix + p + 24);
  c->x[4] = load_littleendian(matrix + p + 32);
  c->x[5] = load_littleendian(matrix + p + 40);
  c->x[6] = load_littleendian(matrix + p + 48);
  c->x[7] = load_littleendian(matrix + p + 56);
  c->x[8] = 0;
  c->x[9] = 0;
}
