...
int crypto_dh_keypair(unsigned char* pk, unsigned char *sk)
{
	...
	ec_fp_smul_256h((uni)pk, (uni)(pk + (PUBLICKEY_BYTES/2)), (uni)zzn, (uni)sk, (uni)xn0, (uni)yn0);
#else
	ec_fp_smulbase_256h((uni)pk, (uni)(pk + (PUBLICKEY_BYTES/2)), (uni)zzn, (uni)sk, (uni *)xn0, (uni *)yn0);
#endif
...
int crypto_dh(unsigned char *s, const unsigned char* pk, const unsigned char *sk){
	...
	ec_fp_smul_256h((uni)result, yyn, zzn, (uni)sk, (uni)pk, (uni)(pk + (PUBLICKEY_BYTES/2)));
...
#ifdef TEST_BOX
void test_try_ec_fp_smulbase_256h()
{
	...
	printf("\nTwisted Hessian (a=1) H (fixedbase)\n");

	for(count = 0; count < 100000000UL; count++){
