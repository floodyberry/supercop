...
wi=j/64;
wx=((mr_small)1<<(j%64));
        if (u0>0) {u0p->w[wi]|=wx; if (u0p->len<wi+1) u0p->len=wi+1;}
        if (u0<0) {u0m->w[wi]|=wx; if (u0m->len<wi+1) u0m->len=wi+1;}
        if (u1>0) {u1p->w[wi]|=wx; if (u1p->len<wi+1) u1p->len=wi+1;}
        if (u1<0) {u1m->w[wi]|=wx; if (u1m->len<wi+1) u1m->len=wi+1;}
...
//        if (u1<0) mr_addbit(_MIPP_ u1m,j);
		...
		mr_mip->w2->w[0]>>=1;
		mr_mip->w2->w[0]|=(mr_mip->w2->w[1]<<M1);
		mr_mip->w2->w[1]>>=1;
		if (mr_mip->w2->w[1]==0)
		{
			mr_mip->w2->len=1;
			if (mr_mip->w2->w[0]==0) mr_mip->w2->len=0;
		}
	

		mr_mip->w3->w[0]>>=1;
		mr_mip->w3->w[0]|=(mr_mip->w3->w[1]<<M1);
		mr_mip->w3->w[1]>>=1;
		if (mr_mip->w3->w[1]==0)
		{
			mr_mip->w3->len=1;
			if (mr_mip->w3->w[0]==0) mr_mip->w3->len=0;
		}
