...
{ /* general purpose interleaving algorithm engine for multi-exp */
    int i,j,tba[4],pba[4],na[4],tbb[4],pbb[4],nb[4],nbits,nbs,nzs;
    int sza,szb,nadds;
#ifdef MR_OS_THREADS
    miracl *mr_mip=get_mip();
#endif
    ...
    nbits=0;
    for (i=0;i<wa;i++) {tba[i]=0; j=logb2(_MIPP_ ma3[i]); if (j>nbits) nbits=j; }
    for (i=0;i<wb;i++) {tbb[i]=0; j=logb2(_MIPP_ mb3[i]); if (j>nbits) nbits=j; }
...
int ecn2_mul2(_MIPD_ big a,ecn2 *FT,big b,ecn2 *Q,ecn2 *R)
{
    ...
    premult(_MIPP_ a,3,a3); /* calculate for NAF */
	premult(_MIPP_ b,3,b3); /* calculate for NAF */
...
{
    int i,j,k,t,len,maxsize,promptr;
    ecn2 w,z;
    ...
    if (size(e[0])<0 || size(e[1])<0) mr_berror(_MIPP_ MR_ERR_NEG_POWER);
    t=MR_ROUNDUP(B->max,B->window);
