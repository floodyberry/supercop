...
#define CHUNK_SIZE 128			// Process checksum after 128 blocks(for TriviA-128 version) // CHANGE
#define MAX_IT 0x00400000          // Intermediate Tag (if any) upto 2^22 blocks (2^32 bytes) 
...
// Update the Tag for the Final 4 Checksum Blocks for AD and Get the Key for Variable Length Handling
  // for(i=0; i<4; i++){process_block(Checksum[i], Tag, Checksum, SC_State, Z+i, PROC_CHECKSUM);}
...
//--------------------------------//
   ...
   while(mlen >=8) // Process ALL but the Final Block of Message 
   {
	...
	Block = load64(m);    
	process_block(Block, Tag, Checksum, SC_State, Z, PROC_BLOCK);
	...
	if(ck_ctr==0) // process the checksum blocks
	   ...
	   ck_ctr=CHUNK_SIZE_MSG; set_zero64(Checksum,3);
...
// Process the Final Block (possibly emty) of Mesage after we apply 10* pad
   process_block(pad(m, mlen), Tag, Checksum, SC_State, Z, PROC_BLOCK);
...
// ...And the final 4 Checksum Blocks for AD
  // for(i=0; i<3; i++){process_block(Checksum[i], Tag, Checksum, SC_State, Z+i, PROC_CHECKSUM);}
 for(i=0; i<3; i++)
	{
	    process_block(Checksum[i], Tag, Checksum, SC_State, Z, PROC_CHECKSUM);
	    if(i == 0) {Tag[0] ^= (u32)(Z[0]>>32); Tag[1] ^= (u32)(Z[0]);}
...
// Process the Final Block (possibly empty) of cipheretxt 
   ...
   process_block(pad(m, clen-16), Tag, Checksum, SC_State, Z, PROC_BLOCK);
   *outputmlen=*outputmlen+clen-16; c=c+clen-16;
// ...And the final 3 Checksum Blocks for AD
   //for(i=0; i<3; i++){process_block(Checksum[i], Tag, Checksum, SC_State, Z+i, PROC_CHECKSUM);}
 for(i=0; i<3; i++)
	{
	    process_block(Checksum[i], Tag, Checksum, SC_State, Z, PROC_CHECKSUM);
