/* PAEQ-160: optimized (AES-NI) version*/
...
#define CRYPTO_MBLOCK (64-D_BYTES-CRYPTO_KEYBYTES)  //42 for 20-byte key
#define CRYPTO_ADBLOCK (64-D_BYTES-2*CRYPTO_KEYBYTES) //22 for 20-byte key
#define CRYPTO_COUNTERBYTES (64-D_BYTES-CRYPTO_KEYBYTES-CRYPTO_NPUBBYTES)  //22 for 20-byte key and 20-byte nonce
...
int PAEQ160_opt_AESNI_decrypt(unsigned char *m, unsigned long long *mlen,
	...
	const unsigned char *k)
{
	...
	__m128i key_mask = _mm_set_epi32(0xffffffff, 0, 0, 0);
	__m128i lastinput_mask = _mm_set_epi32(0, 0xffffffff, 0xffffffff, 0xffffffff);
	...
	//1.3 Setting first block: Constant (bytes 0-1) || Counter (bytes 2-23) || Nonce (bytes 24-43) || Key (bytes 44-63)
	//1.3.1 Domain separation
	BlockInput[0] = D0;  //Domain separation constant

	//1.3.2 Nonce (bytes 24-43) and key (bytes 44-63)
	__m128i Key[2];
	Key[0] = _mm_set_epi64x(*((uint64_t*)k), 0);
	Key[0] = _mm_slli_si128(Key[0], 4);
	Key[1] = _mm_set_epi64x(*((uint64_t*)(k + 12)), *((uint64_t*)(k + 4)));
	__m128i Nonce[2];
	Nonce[0] = _mm_set_epi64x(*((uint64_t*)npub), 0);
	Nonce[1] = _mm_set_epi64x(*((uint32_t*)(npub + 16)), *((uint64_t*)(npub + 8)));
	...
	if (clen != 0)
	{
		...
		while ((clen>CRYPTO_ABYTES))
		{
			...
			if ((clen < (2 * CRYPTO_MBLOCK + CRYPTO_ABYTES)) || (decrypted_bytes == 0))
			{
				...
				//3. Nonce and key
				BlockInput[3] = Key[1];
				BlockInput[2] = _mm_xor_si128(Key[0], Nonce[1]);
				BlockInput[1] = Nonce[0];
				...
				{
					padding[0] = padding[1] = padding[2] = _mm_set1_epi8(clen - CRYPTO_ABYTES);  //X everywhere
					padding[2] = _mm_and_si128(padding[2], lastinput_mask);   
					ciphertext128[0] = _mm_xor_si128(BlockMiddle[0], padding[0]);
					ciphertext128[1] = _mm_xor_si128(BlockMiddle[1], padding[1]);
					ciphertext128[2] = _mm_xor_si128(BlockMiddle[2], padding[2]); //X||F(K)
				...
				memset(plaintext_buffer, 0, 3 * sizeof(__m128i));
				...
				BlockOutput[2] = _mm_and_si128(BlockOutput[2], lastinput_mask);
			...
			}//end of <126 byte condition


			else //More than 125 bytes to encrypt
			{
				...
				BlockExtraMiddle[2] = _mm_and_si128(BlockExtraMiddle[2], key_mask);
				__m128i BlockExtraMiddleTmp = _mm_loadu_si128((__m128i*)(c + decrypted_bytes + 26));  //Bytes [26:41]
				BlockExtraMiddleTmp = _mm_srli_si128(BlockExtraMiddleTmp, 4);   //Bytes [30:41]
				BlockExtraMiddle[2] = _mm_xor_si128(BlockExtraMiddle[2], BlockExtraMiddleTmp);
				...
				BlockMiddle[2] = _mm_and_si128(BlockMiddle[2], key_mask);
				__m128i BlockMiddleTmp = _mm_loadu_si128((__m128i*)(c + decrypted_bytes + 26 + CRYPTO_MBLOCK)); 
				BlockMiddleTmp = _mm_srli_si128(BlockMiddleTmp, 4); //Bytes [30:41]
				BlockMiddle[2] = _mm_xor_si128(BlockMiddle[2], BlockMiddleTmp);
				...
				memcpy(m + decrypted_bytes, ((char*)plaintext_buffer) + 2, CRYPTO_MBLOCK);
				memcpy(m + decrypted_bytes + CRYPTO_MBLOCK, ((char*)plaintext_buffer) + 2+3*sizeof(__m128i), CRYPTO_MBLOCK);
				...
				BlockExtraOutput[2] = _mm_and_si128(BlockExtraOutput[2], lastinput_mask);
	...
	if (adlen != 0)
	{
		...
		//Preparing the block: Constant (bytes 0-1) || Counter (bytes 2-21) || Data (bytes 22-43) || Key (bytes 44-63)
		...
		while (adlen>0)
		{
			if ((adlen < (2 * CRYPTO_ADBLOCK)) || (auth_bytes == 0)) //It is inconvenient or impossible to run two FPerms in parallel
			{
				...
				//2. Counter
				BlockInput[0] = _mm_add_epi64(BlockInput[0], counter_one128);
				BlockInput[0] = _mm_and_si128(BlockInput[0], counterD_mask); //Zeroing bytes 
				BlockInput[1] = _mm_set_epi64x(0, 0);
				BlockInput[3] = Key[1];
				BlockInput[2] = Key[0];

				//3. AD block

				//3.1 Filling AD buffers (block bytes 22-43)
				uint64_t ad_buffer[4];
				ad_buffer[0] = 0;
				if (adlen >= CRYPTO_ADBLOCK)//Full block encryption
					memcpy(((char*)ad_buffer) + 6, ad + auth_bytes, CRYPTO_ADBLOCK);
				else
				{
					//Filling buffers with adlen
					ad_buffer[0] = (((uint64_t)adlen * 0x01010000) << 32);
					for (unsigned i = 1; i < 4; ++i)
						ad_buffer[i] = (((uint64_t)adlen * 0x01010101) << 32) ^ ((uint64_t)adlen * 0x01010101);
					memcpy(((char*)ad_buffer) + 6, ad + auth_bytes, adlen);
				}

				//3.2 Composing BlockInput
				__m128i first_adblock = _mm_set_epi64x(ad_buffer[1], ad_buffer[0]);
				__m128i last_adblock = _mm_set_epi64x(ad_buffer[3], ad_buffer[2]);
				last_adblock = _mm_and_si128(last_adblock, lastinput_mask);
				BlockInput[1] = _mm_add_epi64(BlockInput[1], first_adblock);
				BlockInput[2] = _mm_and_si128(BlockInput[2], key_mask);
				BlockInput[2] = _mm_add_epi64(BlockInput[2], last_adblock);
			...
			else //We authenticate 92-byte chunks
			{
				...
				BlockExtraInput[3] = BlockInput[3] = Key[1];
				BlockExtraInput[2] = BlockInput[2] = Key[0];

				BlockExtraInput[2] = BlockInput[2]; //Key is constant
				BlockExtraInput[3] = BlockInput[3]; //Key is constant
				BlockExtraInput[2] = _mm_and_si128(BlockExtraInput[2], key_mask);
				BlockInput[2] = _mm_and_si128(BlockInput[2], key_mask);

				//2. Loading AD (block bytes 22-43)
				__m128i first_adblock = _mm_loadu_si128((__m128i*)(ad + auth_bytes)); //Loading 16 bytes of AD
				BlockExtraInput[1] = _mm_slli_si128(first_adblock, 6);	//Keeping only 10 bytes of AD (0:6)
				first_adblock = _mm_loadu_si128((__m128i*)(ad + auth_bytes + CRYPTO_ADBLOCK)); //Loading 16 bytes of AD
				BlockInput[1] = _mm_slli_si128(first_adblock, 6);	//Keeping only 10 bytes of AD 


				__m128i last_adblock = _mm_loadu_si128((__m128i*)(ad + auth_bytes + 6)); //Loading 16 bytes of AD
				last_adblock = _mm_srli_si128(last_adblock, 4);	//Keeping only 12 bytes of AD 
				BlockExtraInput[2] = _mm_xor_si128(last_adblock, BlockExtraInput[2]);
				last_adblock = _mm_loadu_si128((__m128i*)(ad + auth_bytes + 6 + CRYPTO_ADBLOCK)); //Loading 16 bytes of AD
				last_adblock = _mm_srli_si128(last_adblock, 4);	//Keeping only 6 bytes of AD 
				BlockInput[2] = _mm_xor_si128(last_adblock, BlockInput[2]);
	...
	// Tag production 
	BlockLastInput[2] = _mm_and_si128(BlockLastInput[2], lastinput_mask);
	BlockLastInput[2] = _mm_xor_si128(BlockLastInput[2], Key[0]);
	BlockLastInput[3] = Key[1];
	...
	Tag[3] = _mm_xor_si128(Tag[3], Key[1]);
	Tag[2] = _mm_xor_si128(Tag[2], Key[0]);
	...
	if (mlen != 0)
	{
		...
		while ((mlen>0))
		{
			...
			if ((mlen < 2 * CRYPTO_MBLOCK) || (encrypted_bytes == 0))
			{
				...
				memset(plaintext_buffer, 0, sizeof(uint64_t)* 6);
				__m128i ciphertext_buffer[3];
				memset(ciphertext_buffer, 0, sizeof(__m128i) * 3);
				...
				plaintext128[2] = _mm_and_si128(plaintext128[2], lastinput_mask);
				...
				memset(ciphertext_buffer, 0, 3 * sizeof(__m128i));
			...
			else //More than 125 bytes to encrypt
			{
				...
				//3. Loading plaintext
				__m128i plaintext_buffer[6];  //128-byte storage
				...
				plaintext_buffer[2] = _mm_loadu_si128((__m128i*)(m + encrypted_bytes + 26));  //Bytes [26:41]
				plaintext_buffer[2] = _mm_srli_si128(plaintext_buffer[2],4); //Right  shift to keep bytes [30:41] only
				plaintext_buffer[3] = _mm_loadu_si128((__m128i*)(m + encrypted_bytes -2 + CRYPTO_MBLOCK));  //Bytes [0:13]
				plaintext_buffer[4] = _mm_loadu_si128((__m128i*)(m + encrypted_bytes + 14+CRYPTO_MBLOCK));   //Bytes [14:29]
				plaintext_buffer[5] = _mm_loadu_si128((__m128i*)(m + encrypted_bytes + 26 + CRYPTO_MBLOCK));  
				plaintext_buffer[5] = _mm_srli_si128(plaintext_buffer[5],4); //Right  shift to keep bytes [30:41] only
				...
				plaintext_buffer[3] = _mm_and_si128(plaintext_buffer[3], not_Dmask); //Clearing first two bytes
...
{
	return PAEQ160_opt_AESNI_encrypt(c, clen, m, mlen, ad, adlen, npub, k);
}
...
{
	return PAEQ160_opt_AESNI_decrypt(m, mlen, c, clen, ad, adlen, npub, k);
}
