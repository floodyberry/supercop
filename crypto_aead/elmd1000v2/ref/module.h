...
/* ===========  AES Key Scheduling for 6 and 10 rounds ============ */
...
static void key_schedule( const u8 *key)
{
	...
	{
		AES_set_encrypt_key_6(key, 128, &aes_key1);
		AES_set_decrypt_key_6(key, 128, &aes_key2);
	}

}

/*=== AES Function for round 6 and 10 : For 6 round AES, Full 6 round Encryption/Decryption is done. === */
static void AES(int r, u8 Is_Encrypt, block res, block inp, AES_KEY *key)
{
	...
	{
		if(Is_Encrypt == ENCRYPT )
			AES_encrypt_r6(inp, res, key);
		else
			AES_decrypt_r6(inp, res, key);
	} 
} 
...
static void process_block(
	...
	/* ==============  AES Encrypt =============== */
	if(type_block == AD)  { AES(rd, ENCRYPT, X, XX, &aes_key1); } 
	if(type_block == MESSAGE) { AES(rd, ENCRYPT, X, XX, &aes_key1); }				
	if(type_block == CIPHERTEXT){  AES(rd, ENCRYPT, X, XX, &aes_key1); } 

	/* ============= Linear Mixing =============== */
	linear_mix(W, Y, X, Is_Encrypt, Is_final);
	
	/* ============= AES Decrypt ================== */
	if(type_block == MESSAGE) {  AES(rd, DECRYPT, YY, Y, &aes_key2); }   
	if(type_block == CIPHERTEXT){  AES(rd, DECRYPT, YY, Y, &aes_key2); }
