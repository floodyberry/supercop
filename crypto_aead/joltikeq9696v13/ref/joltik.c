...
static void set_block_number_in_tweak(uint8_t *tweak, const uint64_t block_no) {
    tweak[ 4] = ((block_no >> 56ULL) & 0x0f);
    tweak[ 5] = ((block_no >> 48ULL) & 0xff);
    tweak[ 6] = ((block_no >> 40ULL) & 0xff);
                         ...
                         uint8_t *ciphertext, size_t *c_len)
{
     ...
     */
    i = 0;
    set_stage_in_tweak(tweak, 0x8 );
    
    hblock = get_block_number_from_tag( tag );
    while (8*(i+1) <= m_len) {
        set_block_number_in_tweak(tweak, hblock );
        set_tweak_in_tweakey(tweakey, tweak);
