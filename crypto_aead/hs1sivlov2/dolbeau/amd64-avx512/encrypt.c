...
#error "This code requires AVX512F to work"
#endif
...
/* custom version of chacha/8rnds generating 128 bytes, with no input message to XOR in */
static inline void chacha_noxor128(chacha_ctx_t *x_, unsigned char *c_)
{
  unsigned i;
...
/* custom version of chacha/8rnds generating 8 bytes, with no input message to XOR in */
static inline void chacha_noxor8(chacha_ctx_t *x_, unsigned char *c_)
{
  unsigned i;
...
/* standard chacha/8rnds, arbitrary lenght & XOR with a message */
...
static inline void hs1siv_chacha256(void *out, const unsigned outbytes, const unsigned char *iv, const void *user_key)
{
    ...
    /* BEWARE: this assume sizeof(hs1siv_ctx_t) == 128 */
    chacha_noxor128(&ctx,out);
}
...
static inline void prf_hash2_1_finalize(uint64_t *h, const uint64_t s[4]) {
    uint64_t s0_1, s1_1;
...
static inline void prf_hash2_1(uint64_t s[4], const uint32_t *in,  unsigned inbytes, const uint32_t *nhkey, const uint64_t *polykey) {
...
#define BLOCKADD(s, I, A, B)                                         \
    __m512i av00##I##s = _mm512_add_epi32(inv, kv##A);               \
    __m512i av04##I##s = _mm512_add_epi32(inv, kv##B)
#define BLOCKUNPACK(s, I)                                               \
    __m512i av00lo64##I##s = _mm512_unpacklo_epi32(av00##I##s, _mm512_setzero_si512()); \
    __m512i av00hi64##I##s = _mm512_unpackhi_epi32(av00##I##s, _mm512_setzero_si512()); \
    __m512i av04lo64##I##s = _mm512_unpacklo_epi32(av04##I##s, _mm512_setzero_si512()); \
    __m512i av04hi64##I##s = _mm512_unpackhi_epi32(av04##I##s, _mm512_setzero_si512())
#define BLOCKMUL(s, I)                                                  \
    __m512i p0_##I##s = _mm512_mul_epu32(av00lo64##I##s, av00hi64##I##s); \
    __m512i p1_##I##s = _mm512_mul_epu32(av04lo64##I##s, av04hi64##I##s)
    
#define BLOCK(s, I, A, B)                                               \
    BLOCKADD(s, I, A, B);                                               \
    BLOCKUNPACK(s, I);                                                  \
    BLOCKMUL(s, I)

#define printv16b(p,v)                                                     \
  do {                                                                  \
        unsigned long long temp[8] __attribute__ ((aligned(64)));             \
        _mm_store_si128((__m128i*)(temp), v);                           \
        int z;                                                          \
        printf("%s: %s: ",p,#v);                                      \
        for (z = 1 ; z >= 0 ; z--) printf("0x%016llx ", temp[z]);          \
        printf("\n");                                                   \
      } while (0)
#define printv64b(p,v)                                                     \
  do {                                                                  \
        unsigned long long temp[8] __attribute__ ((aligned(64)));             \
        _mm512_store_si512((__m512i*)(temp), v);                           \
        int z;                                                          \
        printf("%s: %s: ",p,#v);                                      \
        for (z = 7 ; z >= 0 ; z--) printf("0x%016llx ", temp[z]);          \
        printf("\n");                                                   \
      } while (0)
...
#define SUM(s, I)
    ...
    /* For the reduction this use _mm512_reduce_add_epi64; this
       intrinsic doesn't generate a single instruction,
       but instead a sequence... which is different between
       SKX and KNL. Hopefully, it's the best for each
       uarch... */
#define ACCANDSTORE(s, I)                                               \
    ia0[0] = _mm512_reduce_add_epi64(p0_##I##s) & m60;     \
    ia0[1] = _mm512_reduce_add_epi64(p1_##I##s) & m60;     \
    s0_##I = poly_step(s0_##I, ia0[0], polykey[(2*I)-2]);               \
    s1_##I = poly_step(s1_##I, ia0[1], polykey[(2*I)-1])
    
#define ACCANDSTORENOPOLY(s, I)                                               \
    ia##I[s][0] = _mm512_reduce_add_epi64(p0_##I##s) & m60;     \
    ia##I[s][1] = _mm512_reduce_add_epi64(p1_##I##s) & m60
...
#define ALL1HASH_SEQ(s)                             \
    BLOCK(s, 1, 0,  4);                             \
    SUM(s, 1);                                      \
    ACCANDSTORE(s, 1)
    
#define ALL1HASH_INTER(s)                           \
    BLOCK(s, 1, 0,  4);                             \
    SUM(s, 1);                                      \
    ACCANDSTORE(s, 1)

#define ALL1HASH(s) ALL1HASH_SEQ(s)

#define ALL1HASHNOPOLY(s)                           \
    BLOCK(s, 1, 0,  4);                             \
    SUM(s, 1);                                      \
    ACCANDSTORENOPOLY(s, 1)

#define ALL1HASHPOLYONLY(s)                     \
    POLYONLY(s, 1)

#if 0
    /* Hash full blocks, unrolled by 4 */
    /* should be tested on the real hardware */
    for ( ; inbytes >= HS1_SIV_NH_LEN*4; inbytes -= HS1_SIV_NH_LEN*4, in += HS1_SIV_NH_LEN) {
      ALIGN32 uint64_t ia1[4][2];

          __m512i kv0  = _mm512_loadu_si512((const __m512i*)(nhkey+ 0)); // 1
          __m512i kv4  = _mm512_loadu_si512((const __m512i*)(nhkey+ 4)); // 1
          __m512i kv8  = _mm512_loadu_si512((const __m512i*)(nhkey+ 8)); // 1
          __m512i kv12 = _mm512_loadu_si512((const __m512i*)(nhkey+12)); // 1

          __m512i inv = _mm512_loadu_si512((const __m512i*)(in+ 0));
          
          ALL1HASHNOPOLY(0);
          inv = _mm512_loadu_si512((const __m256i*)(in+ 16));
          ALL1HASHNOPOLY(1);
          inv = _mm512_loadu_si512((const __m256i*)(in+ 32));
          ALL1HASHNOPOLY(2);
          inv = _mm512_loadu_si512((const __m256i*)(in+ 48));
          ALL1HASHNOPOLY(3);
...
static inline void hs1_data(unsigned char tmp_t[HS1_SIV_SIV_LEN], 
  ...
  if (arnd64 > 0)
    prf_hash2_1(s, (uint32_t *)a, arnd64, (uint32_t *)hs1_ctx->nh_key, (uint64_t *)hs1_ctx->poly_key);
  /* last one if needed */
  if (abytes > arnd64) {
    memset(tmp_h,0,HS1_SIV_NH_LEN);
    for (i = 0 ; i < (abytes-arnd64) ; i++)
      tmp_h[i] = ((unsigned char*)a)[arnd64+i];
    prf_hash2_1(s, (uint32_t *)tmp_h, HS1_SIV_NH_LEN, (uint32_t *)hs1_ctx->nh_key, (uint64_t *)hs1_ctx->poly_key);
  }
  donesizes = 0;
  /* hash message M */
  if (mbytes > 0) {
    ...
    if (mrnd64 > 0)
      prf_hash2_1(s, (uint32_t *)((unsigned char*)m), mrnd64, (uint32_t *)hs1_ctx->nh_key, (uint64_t *)hs1_ctx->poly_key);
    ...
    prf_hash2_1(s, (uint32_t *)tmp_h, k, (uint32_t *)hs1_ctx->nh_key, (uint64_t *)hs1_ctx->poly_key);
  }
  ...
  if (!donesizes) {
    ...
    donesizes = 1;
    prf_hash2_1(s, (uint32_t *)tmp_h, 16, (uint32_t *)hs1_ctx->nh_key, (uint64_t *)hs1_ctx->poly_key);
  }
  prf_hash2_1_finalize(h,s);
}
...
static inline void hs1siv_encrypt(hs1siv_ctx_t *hs1_ctx, void *m, unsigned mbytes, void *a, unsigned abytes, void *n, void *t, void *c)
{
    ...
    /* hardwire the non-xor & HS1_SIV_SIV_LEN == 8 */
    chacha_noxor8(&chacha_ctx, (unsigned char *)tmp_t);
    s[0] = s[1] = 1;
    prf_hash2_1(s, (uint32_t *)tmp_t, HS1_SIV_SIV_LEN, (uint32_t *)hs1_ctx->nh_key, (uint64_t *)hs1_ctx->poly_key);
    prf_hash2_1_finalize(h,s);
    memcpy(key, hs1_ctx->chacha_key, 32);
    for (i=0; i<sizeof(h)/8;i++)
       ((uint64_t *)key)[i] ^= h[i];
...
static inline int hs1siv_decrypt(hs1siv_ctx_t *ctx, void *c, unsigned cbytes, void *a, unsigned abytes, void *n, void *t, void *m)
{
    ...
    int r;
    s[0] = s[1] = 1;
    prf_hash2_1(s, (uint32_t *)t, HS1_SIV_SIV_LEN, (uint32_t *)ctx->nh_key, (uint64_t *)ctx->poly_key);
    prf_hash2_1_finalize(h,s);
    ...
    chacha_ivsetup(&chacha_ctx,(unsigned char *)n);
    /* hardwire the non-xor & HS1_SIV_SIV_LEN == 8 */
    chacha_noxor8(&chacha_ctx, (unsigned char *)maybe_t);
...
)
{
    ...
    assert(sizeof(hs1siv_ctx_t) == 128);
    (void)nsec;
