...
static inline void prf_hash2_1_finalize(uint64_t *h, const uint64_t s[2]) {
    uint64_t s0_1, s1_1;
...
static inline void prf_hash2_1(uint64_t s[2], const uint32_t *in,  unsigned inbytes, const uint32_t *nhkey, const uint64_t *polykey) {
    ...
    for ( ; inbytes >= HS1_SIV_NH_LEN; inbytes -= HS1_SIV_NH_LEN, in += HS1_SIV_NH_LEN/4) {
      ALIGN32 uint64_t ia0[4];
        ...
        // assume HS1_SIV_NH_LEN == 64 -> 2 iterations -> full unroll, less scalar reduction
          ...
          __m256i kv8  = _mm256_loadu_si256((const __m256i*)(nhkey+ 8)); // 1
          __m256i kv12 = _mm256_loadu_si256((const __m256i*)(nhkey+12)); // 1
