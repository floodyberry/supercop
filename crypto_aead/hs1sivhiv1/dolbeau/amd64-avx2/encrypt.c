...
// Current version : $Date: 2014/08/22 16:53:57 $
//
...
#define ALL3HASHPOLYONLY(s)                     \
    ...
    /* Doesn't seem to help (hurts, even); kept FYI */
    for ( ; inbytes >= HS1_SIV_NH_LEN*4; inbytes -= HS1_SIV_NH_LEN*4, in += HS1_SIV_NH_LEN) {
      ...
      ALIGN32 uint64_t ia3[4][2];
          ...
          __m256i kv16 = _mm256_loadu_si256((const __m256i*)(nhkey+16)); // 2, 3
          __m256i kv20 = _mm256_loadu_si256((const __m256i*)(nhkey+20)); // 2, 3
          __m256i kv24 = _mm256_loadu_si256((const __m256i*)(nhkey+24)); // 3
          __m256i kv28 = _mm256_loadu_si256((const __m256i*)(nhkey+28)); // 3

          __m256i inva = _mm256_loadu_si256((const __m256i*)(in+ 0));
          __m256i invb = _mm256_loadu_si256((const __m256i*)(in+ 8));
          
          ALL3HASHNOPOLY(0);
          inva = _mm256_loadu_si256((const __m256i*)(in+ 16));
          invb = _mm256_loadu_si256((const __m256i*)(in+ 24));
          ALL3HASHNOPOLY(1);
          inva = _mm256_loadu_si256((const __m256i*)(in+ 32));
          invb = _mm256_loadu_si256((const __m256i*)(in+ 40));
          ALL3HASHNOPOLY(2);
          inva = _mm256_loadu_si256((const __m256i*)(in+ 48));
          invb = _mm256_loadu_si256((const __m256i*)(in+ 56));
          ALL3HASHNOPOLY(3);
...
static inline void hs1_data(unsigned char tmp_t[HS1_SIV_SIV_LEN], 
  ...
  /* create & hash junction block */
  donesizes = 0;
  memset(tmp_h,0,HS1_SIV_NH_LEN);
  for (i = 0 ; i < (abytes-arnd64) ; i++)
    tmp_h[i] = ((unsigned char*)a)[arnd64+i];
  j = ((atot-arnd64)+15)&~15;
  borrowm = HS1_SIV_NH_LEN - j; // 0,16,32,64
  if (borrowm > mbytes)
    borrowm = mbytes;
  for (i = 0 ; i < borrowm  ; i++) {
    tmp_h[i+j] = ((unsigned char*)m)[i];
  }
  k = (j+borrowm+15)&~15;
  if (k < 64) {
  ...
  if (borrowm < mbytes) {
    ...
    mrnd64 = (mbytes-borrowm)&~63;
    if (mrnd64 > 0)
      prf_hash2_3(s, (uint32_t *)(((unsigned char*)m)+borrowm), mrnd64, (uint32_t *)hs1_ctx->nh_key, (uint64_t *)hs1_ctx->poly_key);
    /* hash last block of M, perhaps with sizes */
    memset(tmp_h,0,HS1_SIV_NH_LEN);
    j = mbytes - (mrnd64+borrowm);
    for (i = 0 ; i < j ; i++)
      tmp_h[i] = ((unsigned char*)m)[mrnd64+borrowm+i];
    k = (j+15)&~15;
    if (k < 64) {
      (( uint64_t*)tmp_h)[2*(k/16)] = abytes;
      (( uint64_t*)tmp_h)[2*(k/16)+1] = mbytes;
