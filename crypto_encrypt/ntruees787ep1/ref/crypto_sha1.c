                                    ...
                                    //          may be NULL if not FINISH
{
    ...
    if (in_len) {
...

        {
            ...
            if ((c->num_bits_hashed[1] += bits1) < bits1) {
                memset((uint8_t *) c, 0, sizeof(SHA1_CTX));
                space = 0;
    ...
    if (flags & SHA_FINISH) {
        ...
        crypto_uint32_2_msbyte(md, c->state, 5);
...
crypto_sha1_init(
    SHA1_CTX   *c)                  // in/out - pointer to SHA-1 context
{
    return crypto_sha1(c, NULL, NULL, 0, SHA_INIT, NULL);
}
...
{
    return crypto_sha1(c, NULL, data, data_len, SHA_DATA_ONLY, NULL);
}
...
crypto_sha1_final(
    ...
    uint8_t    *md)                 //   out - address for message digest
{
    return crypto_sha1(c, NULL, NULL, 0, SHA_FINISH, md);
}
...
crypto_sha1_final_zero_pad(
    ...
    uint8_t    *md)                 //   out - address for message digest
{
    return crypto_sha1(c, NULL, NULL, 0, SHA_FINISH | SHA_ZERO_PAD, md);
}
    ...
    uint8_t        *md)             // out - address for message digest
{
    ...
    return crypto_sha1(&c, NULL, data, data_len, SHA_INIT | SHA_FINISH, md);
}
