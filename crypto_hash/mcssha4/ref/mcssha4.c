...
// MCSSHA4_MBLOCK = 64
// MCSSHA4_LBLOCK = 128
// SR length for h = 224 and 256 is MCSSHA4_MBLOCK (64 bytes)
// SR length for h = 384 and 512 is MCSSHA4_LBLOCK (128 bytes)
HashReturn Init(hashState *c,DataLength hashbitlen)
	{
		...
		{
		    c->x[2] = MCSSHA4_MBLOCK - 4;
		    c->x[3] = MCSSHA4_MBLOCK - 1;
		    for(i=0;i<MCSSHA4_MBLOCK;i++)c->data[i] = i;
		}
		else
		{
		    c->x[2] = MCSSHA4_LBLOCK - 4;
		    c->x[3] = MCSSHA4_LBLOCK - 1;
		    for(i=0;i<MCSSHA4_LBLOCK;i++)c->data[i] = i;
		}
...
// SR length for h = 224 and 256 is 64 bytes (MCSSHA4_MBLOCK)
// SR length for h = 384 and 512 is 128 bytes (MCSSHA4_LBLOCK)
...
// Prepare input sequence: double SR state from pre-hash computation
        if(c->hashbitlen == 224 | c->hashbitlen == 256)
		{
		    memcpy(data,c->data,MCSSHA4_MBLOCK);
		    memcpy(data+MCSSHA4_MBLOCK,c->data,MCSSHA4_MBLOCK);
		}
		else
		{
		    memcpy(data,c->data,MCSSHA4_LBLOCK);
		    memcpy(data+MCSSHA4_LBLOCK,c->data,MCSSHA4_LBLOCK);
		}
...
//    (MCSSHA4_LBLOCK (128) for h = 224 and 256, and MCSSHA4_2LBLOCK (256) for h = 384 and 512)
		i = 0;
