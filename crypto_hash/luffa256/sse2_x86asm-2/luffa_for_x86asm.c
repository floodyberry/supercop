...
static void Update256(hashState *state, const BitSequence *data, DataLength databitlen) 
{
    ...
    if (state->rembitlen + databitlen >= MSG_BLOCK_BIT_LEN) {
        ...
        while (round_num > 0x00000000ffffffffULL) {
            luffa256(state->chainv, data, 0xffffffff);
            round_num -= 0xffffffff;
        }

        if (round_num > 0) {
            luffa256(state->chainv, data, (uint32)round_num);
        }
...
static void Update384(hashState *state, const BitSequence *data, DataLength databitlen) 
{
    ...
    if (state->rembitlen + databitlen >= MSG_BLOCK_BIT_LEN) {
        ...
        while (round_num > 0x00000000ffffffffULL) {
            luffa384(state->chainv, data, 0xffffffff);
            round_num -= 0xffffffff;
        }

        if (round_num > 0) {
            luffa384(state->chainv, data, (uint32)round_num);
        }
