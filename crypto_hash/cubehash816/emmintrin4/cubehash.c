...
HashReturn Init(hashState *state, int hashbitlen)
{
  ...
  if (hashbitlen == 512) {
    state->x[0] = _mm_set_epi32(0x2de1cbb7,0xc3b76979,0x1f3ba16a,0xe8338f26);
    state->x[1] = _mm_set_epi32(0xe3b10028,0xa8788316,0x67b05ca3,0xa1073e33);
    state->x[2] = _mm_set_epi32(0xd3947fa6,0x425037fa,0xe08cc07a,0xc8a50903);
    state->x[3] = _mm_set_epi32(0x5f3b3111,0x846fd3ad,0x34f263a7,0x679e9fd6);
    state->x[4] = _mm_set_epi32(0xb6a92e1c,0x8ac5eabd,0xf0538448,0x5d1a94e0);
    state->x[5] = _mm_set_epi32(0x9ac78463,0xe72fec1d,0xf099981c,0x6b8697b7);
    state->x[6] = _mm_set_epi32(0x4e06fd40,0x891252e9,0x9359ac56,0x1c076d7b);
    state->x[7] = _mm_set_epi32(0xbb6d3868,0x645cb77c,0x3269a3b5,0x43acc6f7);
  } else if (hashbitlen == 256) {
    state->x[0] = _mm_set_epi32(0x3e8fbad7,0x8f43c8b8,0x61c6f881,0x5a9c5414);
    state->x[1] = _mm_set_epi32(0x3983a463,0x95843afe,0xbc1283c0,0x2b2cf5f3);
    state->x[2] = _mm_set_epi32(0xcc28027a,0x4d9ea1db,0x79d02186,0x8f8096e4);
    state->x[3] = _mm_set_epi32(0xc87653cb,0x6214e13f,0xca323b60,0x16b0e5e7);
    state->x[4] = _mm_set_epi32(0x791c7ca7,0x41bfd52d,0x0f791029,0x2e6dd5f0);
    state->x[5] = _mm_set_epi32(0xc4297393,0xb2586c02,0x22d518d8,0xf745fee9);
    state->x[6] = _mm_set_epi32(0x9be5a7d3,0x022e8a81,0xd1fb2b30,0xb828d06d);
    state->x[7] = _mm_set_epi32(0x00f72e24,0x45ab5aef,0xeb99f3e6,0xeb222ab2);
  } else {
