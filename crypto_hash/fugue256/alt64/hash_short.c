...
crypto_uint32 state[30] = {0,0,0,0,0,0,0,0,
    ...
    crypto_uint32 len_div32, j32, j_small;
    ...
    if ((len_div >>32)==0)
      { /* use 32 bit lengths*/
	len_div32 = len_div& 0xffffffff; j_small = j;
	for (j32=0; j32<len_div32; j32++)
	  {
	    round(j_small,0,in,IMPL_TYPE); j_small++;
	    round(j_small,1,in,IMPL_TYPE); j_small++;
	    round(j_small,2,in,IMPL_TYPE); j_small++;
	    round(j_small,3,in,IMPL_TYPE); j_small++;
	    round(j_small,4,in,IMPL_TYPE); j_small++;
	  }
	j = j_small;
      }
    else
