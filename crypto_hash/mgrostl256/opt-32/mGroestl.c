...
void F512(u32 *h, const u32 *m, u32 *c) {
  int i,j=0;
  u32 Ptmp[2*COLS512];
  u32 Qtmp[2*COLS512];
  u32 y[2*COLS512];
...
void F1024(u32 *h, const u32 *m, u32 *c) {
  int i;
  u32 Ptmp[2*COLS1024];
  u32 Qtmp[2*COLS1024];
  u32 y[2*COLS1024];
...
//-------
  ...
  //divided into 1024 block*///MODIFIED BY GURPREET 
...
HashReturn Init(hashState* ctx,
  ...
  ctx->buffer = malloc(ctx->blocksize);		//modified
...
HashReturn Update(hashState* ctx,
  ...
  if (ctx->buf_ptr) {
    while (ctx->buf_ptr < ctx->blocksize && index < msglen) {//MODIFIED
...
/*for padding block counter*/
 ...
 /* as block)counter1 is modified to u64/// no need for blockcounter2
