...
#define round(state0,state1,state2,state3,state4,state5,state6,state7,x0,x1,x2,x3,y0,y1,y2,y3,rk,position)\
{\
...
void E256(u32 pt[8], u32 ct[8], u32 message[16], u32 counter[2])
{
   ...
   for (round_cnt=0;round_cnt<ExternalRounds;round_cnt++) {
      round(state0,state1,state2,state3,state4,state5,state6,state7,x0,x1,x2,x3,y0,y1,y2,y3,rk,(12*round_cnt)%32);
      if (round_cnt == 1) LinExpansion(rk,i,j,k); 
      if (round_cnt == 3) NonLinExpansion(rk,counter,x0,x1,x2,x3,y0,y1,y2,y3,48);
      if (round_cnt == 3) LinExpansion(rk,i,j,k);
      if (round_cnt == 5) NonLinExpansion(rk,counter,x0,x1,x2,x3,y0,y1,y2,y3,80);
      if (round_cnt == 7) LinExpansion(rk,i,j,k); 
      if (round_cnt == 8) NonLinExpansion(rk,counter,x0,x1,x2,x3,y0,y1,y2,y3,112);
      if (round_cnt == 9) LinExpansion(rk,i,j,k); 
   }	 
