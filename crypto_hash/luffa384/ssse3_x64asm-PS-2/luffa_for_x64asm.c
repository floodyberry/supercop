...
static void Update256(hashState *state, const BitSequence *data, DataLength databitlen) 
{
    ...
    if (state->rembitlen + databitlen >= MSG_BLOCK_BIT_LEN) {
        ...
        if (!state->rembitlen) {
            ...
            for (i=0;i<cpylen;i++) {
                state->buffer[(state->rembitlen >> 3) + i] = data[i];
            }
        }

        luffa256(state->chainv, state->buffer, 1);
    ...
    if (databitlen) {
        ...
        for (i=0;i<len;i++) {
            state->buffer[state->rembitlen/8+i] = data[i];
        }
...
static void Update384(hashState *state, const BitSequence *data, DataLength databitlen) 
{
    ...
    if (state->rembitlen + databitlen >= MSG_BLOCK_BIT_LEN) {
        ...
        luffa384(state->chainv, state->buffer, 1);
...
static void Update512(hashState *state, const BitSequence *data, DataLength databitlen) 
{
    ...
    if (state->rembitlen + databitlen >= MSG_BLOCK_BIT_LEN) {
        ...
        luffa512(state->chainv, state->buffer, 1);
...
HashReturn Final(hashState *state, BitSequence *hashval) 
{
    ...
    switch(state->hashbitlen) {
        case 224:
            ...
            for(i=0;i<28;i++) hashval[i] = output[i];
        ...
        case 256:
            ...
            for(i=0;i<32;i++) hashval[i] = output[i];
        ...
        case 384:
            ...
            for(i=0;i<48;i++) hashval[i] = output[i];
        ...
        case 512:
            ...
            for(i=0;i<64;i++) hashval[i] = output[i];
...
static void process_last_msgs256(hashState *state)
{
    ...
    if (!(tail_len % 8)) {
        ...
        state->buffer[i] &= (0xff << (8 - (tail_len % 8)));
        state->buffer[i] |= (0x80 >> (tail_len % 8));
    }

    i++;

    for (;i<32;i++) state->buffer[i] = 0;
