...
/* compute a round in P or Q (long variants) */
#define RND(x,y,r,s) do {			\
    COLUMN(x,y,15,15, 0, 1, 2, 3, 4, 5,10,0,0);	\
    COLUMN(x,y,14,14,15, 0, 1, 2, 3, 4, 9,0,0);	\
    COLUMN(x,y,13,13,14,15, 0, 1, 2, 3, 8,0,0);	\
    COLUMN(x,y,12,12,13,14,15, 0, 1, 2, 7,0,0);	\
    COLUMN(x,y,11,11,12,13,14,15, 0, 1, 6,0,0);	\
    COLUMN(x,y,10,10,11,12,13,14,15, 0, 5,0,0);	\
    COLUMN(x,y, 9, 9,10,11,12,13,14,15, 4,0,0);	\
    COLUMN(x,y, 8, 8, 9,10,11,12,13,14, 3,0,0);	\
    COLUMN(x,y, 7, 7, 8, 9,10,11,12,13, 2,0,0);	\
    COLUMN(x,y, 6, 6, 7, 8, 9,10,11,12, 1,0,0);	\
    COLUMN(x,y, 5, 5, 6, 7, 8, 9,10,11, 0,0,s);	\
    COLUMN(x,y, 4, 4, 5, 6, 7, 8, 9,10,15,0,0);	\
    COLUMN(x,y, 3, 3, 4, 5, 6, 7, 8, 9,14,0,0);	\
    COLUMN(x,y, 2, 2, 3, 4, 5, 6, 7, 8,13,0,0);	\
    COLUMN(x,y, 1, 1, 2, 3, 4, 5, 6, 7,12,0,0);	\
...
/* hash bit sequence */
int crypto_hash_groestl384_opt64(unsigned char *out,
				 const unsigned char *in,
