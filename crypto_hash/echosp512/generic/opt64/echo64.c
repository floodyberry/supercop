...
HashReturn Init(hashState *state, int hashbitlen)
{
  ...
  if(hashbitlen > 256) 
    state->cv_blocks = ((2048-BLOCK512)/8)/16;
  else 
    state->cv_blocks = ((2048-BLOCK256)/8)/16; 
...
#define FEEDforward do { \
 ...
 state->state[6] =state->CV[6]^ WL11; \
 state->state[7] =state->CV[7]^ WH11; \
 if(state->cv_blocks < 4){ \
   state->state[0] ^= state->state[4] ; \
   state->state[1] ^= state->state[5] ; \
   state->state[2] ^= state->state[6] ; \
   state->state[3] ^= state->state[7] ; \
 }\
} while(0);
#endif
...
int crypto_hash_echosp512_generic_opt64(unsigned char *out,const unsigned char *in,unsigned long long inlen)
{
